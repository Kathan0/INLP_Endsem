...
Works for any number of classes (Task B: 11, Task C: 4, etc.)
"""
...
class CachedDataset(Dataset):
    ...
    def __init__(self, cache_dir: str, split: str):
        ...
        self.features   = torch.load(feat_path, weights_only=True)   # [N, 102]
        self.embeddings = torch.load(emb_path,  weights_only=True)   # [N, 2048]
        meta            = torch.load(meta_path, weights_only=False)

        self.ids        = meta['ids']
        self.has_labels = 'labels' in meta
        if self.has_labels:
            self.labels = meta['labels']          # [N]  (int64, 0-indexed)
