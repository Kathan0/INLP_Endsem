"""
Step 2: Extract StarCoder2-3b backbone embeddings (2048-dim) for all data splits.

Saves to cache/:
  {split}_embeddings.pt  - FloatTensor [N, 2048]

The backbone is loaded once per split to keep GPU memory manageable.
Rows are processed in the same order as extract_features.py (same parquet_indices).
...
def embed_codes(codes, backbone, tokenizer, device, max_length: int, batch_size: int,
    ...
    return torch.stack(all_embeddings)  # [N, 2048]
...
def main():
    parser = argparse.ArgumentParser(description="Extract StarCoder2 embeddings for Task B splits.")
