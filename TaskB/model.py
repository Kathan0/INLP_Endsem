...
Task B: Multi-class authorship attribution (11 classes).

Architecture:
  backbone_embedding [2048]  ──► LayerNorm ──► tanh-Linear ──► [2048]
  handcrafted_features [102] ──► MLP (102→256→128) ──► [128]
                                                               │
                                                     cat [2176] ──► fusion MLP ──► [11]

The backbone is always used frozen (weights loaded 8-bit in extract_embeddings.py;
during training only the head — ~8 M params — is updated).

Classes:
  0: human        1: deepseek-ai   2: qwen       3: 01-ai      4: bigcode
  5: gemma        6: phi           7: meta-llama  8: ibm-granite 9: mistral
  10: openai
"""
...
class TaskBModel(nn.Module):
    def __init__(self, config: dict):
        ...
        # Process handcrafted features (AST + pattern + stylometric from MyProject/src)
        ...
        # Fusion + classifier — hidden layer expanded slightly for harder 11-class task
        num_labels = config['model']['num_labels']  # 11
        ...
        # Move head to same GPU as backbone
        if torch.cuda.is_available():
            head_device = (torch.device("cuda:1") if torch.cuda.device_count() > 1
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if class_weights is not None:
                class_weights = class_weights.to(head_device)
