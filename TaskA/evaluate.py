"""
Evaluate a trained model on the validation set (or test_sample).

Usage:
  python evaluate.py                                    # val set, best_model.pt
  python evaluate.py --split val
  python evaluate.py --split test_sample                # leaderboard sample (has labels)
...
def main():
    ...
    if 'val_acc' in ckpt:
        print(f"  Checkpoint val accuracy (train time): {ckpt['val_acc']:.4f}")
    if 'epoch' in ckpt:
    ...
    print("\nConfusion Matrix:")
    print(f"             Pred 0 (Human)  Pred 1 (AI)")
    print(f"True 0 (Human)   {cm[0][0]:8,}      {cm[0][1]:8,}")
    print(f"True 1 (AI)      {cm[1][0]:8,}      {cm[1][1]:8,}")

    print("\nPer-class report:")
    print(classification_report(labels, preds,
                                target_names=['Human (0)', 'AI (1)'], digits=4))
